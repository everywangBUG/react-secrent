...
export const HeaderCalendar: React.FC<HeaderCalendarProps> = (props) => {
  ...
  return (
    <div flex gap-x-30px p-y-10px items-center>
      <div w-28px h-28px cursor-pointer hover:bg-themeColor text-center rounded-2px onClick={handlePrevMonth}>&lt;</div>
      <div font-700>{dayjs(curMonth).format(CalendarLocale.formatMonth)}</div>
      <div w-28px h-28px cursor-pointer hover:bg-themeColor text-center rounded-2px onClick={handleNextMonth}>&gt;</div>
